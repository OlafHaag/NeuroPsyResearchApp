...
class ScreenInstructCircleTask(BaseScreen):
    ...
    def get_message(self):
        ...
        n_tasks_msg = _("You have {0} task{1} in this block. Be as quick and as accurate as possible while solving the "
                        "task{1}").format(n_tasks, task_suffix)
        time_limit_msg = _("A trial starts by giving you {0} seconds time to prepare for the trial. During this time "
                           "the sliders are inactive and can't be moved. After this preparation phase the sliders "
                           "become active and you'll have {1} seconds time to complete the task{3}. The time "
                           "limitation is visualized by counting down the seconds in the middle of the screen, as "
                           "well as by an outer [color=ff00ff]purple ring[/color] that will close as the time runs "
                           "out. A trial ends when the outer [color=ff00ff]purple ring[/color] closes and the "
                           "countdown reaches 0.\n"
                           "Shortly thereafter ({2} seconds) you will be prompted to get ready for the next trial."
                           ).format(ct.warm_up, ct.trial_duration, ct.cool_down, task_suffix)
        task1_msg = _("Hold your device with both hands so your thumbs can reach their respective side of the "
                      ...
                      "get the [b]white disk[/b] to touch the [color=008000]green ring[/color]."
                      )
